...
class EncoderRNN(nn.Module):
    def __init__(self, input_size, embedding_dim, hidden_size, padding_idx, n_layers=1, dropout = 0.4, bidirectional = True, discriminator=False):
        ...
        self.embedding = nn.Embedding(input_size, embedding_dim, padding_idx=padding_idx) #1. input_ids에 대한 embedding 생성
    ...
    def forward(self, X, attn=None, Rattn=None):
        ...
        if X[0].dim()==2:
            #print(X[0]) #len x batch
            embedded = self.embedding(X[0])
        #    print(embedded.shape) # len x batch x embedding_size
        else:
            embedded = X[0].matmul(self.embedding.weight)

        if Rattn is not None:
            len = attn.size(1)
            embedded = embedded[:len, :,:] * Rattn.transpose(1, 0) # multiply
        ...
        return output, hidden #last hidden state 출력 = step별 hidden state가 전부 출력 됨


class MLPAttention(nn.Module):
    def __init__(self, hidden_dim, att_dim, dropout=0.4): #500 -> 100
        super(MLPAttention, self).__init__()
        self.W_k = nn.Linear(hidden_dim, att_dim, bias=True) #500 -> 100
        self.v = nn.Linear(att_dim, 1, bias=True) #100 -> 1
        self.dropout = nn.Dropout(dropout)
        self.tanh = nn.Tanh()
#         self.linear = nn.Linear(hidden_dim, 2)

    def forward(self, encoder_outputs, lengths, temp=1):
        V = encoder_outputs #len x batch x hidden_dim (16 x 512 x 500)
        #print('V모양 :', V.shape)

        max_len = encoder_outputs.size(0)
        #print('max_len : ',V.size(0))
        ...
        #print('초기 encoder output ', encoder_outputs.shape)
        encoder_outputs = self.W_k(encoder_outputs) #encoder output : input token의 size가 hidden size로 변경
        #print("첫번쨰 ", encoder_outputs.shape)
        encoder_outputs = self.dropout(encoder_outputs)
        #print("두번째 ", encoder_outputs.shape)
        encoder_outputs = self.tanh(encoder_outputs) #hidden state 꼴로 만듦
        #print("세번쨰 ", encoder_outputs.shape)
        scores = self.v(encoder_outputs).squeeze(-1) #linear layer의 weight를 어텐션 연산의 쿼리로 봄 16 x 512 = 각 토큰의 attention score가 계산 됨
        #print('scores ', scores.shape) # length x batch
        ## MASK OUT PAD
        scores = scores.masked_fill(src_mask.T, float("-inf"))
        # print('중간 scores ', scores.shape)
        scores = torch.softmax(scores/temp, dim=0).unsqueeze(0).permute(2, 1, 0) #attention score 512x16x1 => attention weight로 변환
        # print('마지막 scores ', scores.shape) #512 x 16 x 1

        attn_hidden = torch.bmm(V.permute(1, 2, 0), scores) #512 x 500 x 16 , 512 x 16x1 => GRU hidden state에 attention weight 곱해주는 부분 (가중합 시켰다고 보면 됨)=>sentence에 대한 표현
        # print('atten_hidden 값', attn_hidden.shape) #512 x 500 x 1 => classifier의 input으로 들어감
        #scores = batch x len x 1
        #attn_hidden = batch x dim x 1
        reverse_scores = 1-scores.squeeze(2) #구했던 attention score를 1에서 빼줌
...
class SentimentClassifier(nn.Module):
    def __init__(self, hidden_size, num_class=32, dropout=0.3):
    ...
    def forward(self, attn_output):
        #print(self.output(self.linear(attn_output).squeeze()))
        return self.linear(attn_output).squeeze() # batch x hidden

class Generator(nn.Module):
    def __init__(self, enc_cls, mlp, enc):
        super(Generator, self).__init__()
        self.enc_cls = enc_cls #encoder classifier (GRU부분)
        self.mlp = mlp #mlp layer (attention 부분)
        self.enc = enc # encoder (reverse attention이 적용된 input이 들어가는 부분)
...
class DecoderRNN_LN(BaseRNN): #!
    ...
    def __init__(self, vocab_size, max_len, hidden_size,
        ...
        self.gamma = nn.Embedding(num_class, style_size) #2, 200 크기의 embedding #!
        self.beta = nn.Embedding(num_class, style_size) #2, 200 크기의 embedding #!
        #print('beta_size: ', self.beta.weight)
    ...
    def forward(self, inputs=None, encoder_hidden=None, encoder_outputs=None, function=F.log_softmax, teacher_forcing_ratio=0, mask = None, attn_hidden = None, label=None, gumbel=False, max_length = None):
        ret_dict = dict()
        if self.use_attention:
        ...
        #print('attention shape 1, ', attn_hidden.shape)
        attn_hidden = self.reduce(attn_hidden.detach()) #affine transformation
        #print('attention shape 2, ', attn_hidden.shape)

        beta = self.beta(label)
        #print(label)

        gamma = self.gamma(label)
        mean = torch.mean(attn_hidden, -1, keepdim=True) # layer normalization 부분 #!
