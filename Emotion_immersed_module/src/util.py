...
def load_batch_iterator_with_eos(path, train, val, test, batch_size, device):
    ...
    train, dev, test = TabularDataset.splits(path=path, train=train,
                      ...
                      fields={"X":("X", X),
                              "C": ('C', C)})
    print('훈련 샘플 수 : {}'.format(len(train)))
    print('val 수 : {}'.format(len(dev)))
    print('test 수 : {}'.format(len(test)))
    ...
    X.build_vocab(train, specials=["<bos>"], min_freq=2)
    C.build_vocab(train)
    ...
    # print(batch.X[0].permute(1, 0).shape) #len x batch_size

    #print(X.vocab.itos[2])
    return train, dev, test, train_iter, dev_iter, test_iter, X, C

def PLM_tokenizer_with_eos():
    roberta_tokenzier = RobertaTokenizerFast('roberta-large')
    #input train_data,
    #add datasets token  in  PLM_vocab
    #using tokenizer, making datasets, vocab
    pass
...
def runClassifier(batch, enc, attn, cls):
    # RETURN logits
    #print(batch.X[0][:,0]) # length x batch
    output, _= enc(batch.X) # length x batch x hidden_dim
...
def get_classifier(input_size, embedding_dim, attention_dim, hidden_dim, pad_idx, device, data, model_name):
    enc = EncoderRNN(input_size, embedding_dim, hidden_dim, pad_idx).to(device) #모델 인스턴스 생성
    enc.load_state_dict(torch.load(f"model/{data}_enc_{model_name}.pth")) #parameter update
...
def computeOtherLoss(batch, G, dec, enc_r, attn_r, senti_r, hidden_self, BCE, mse, nll, device, reduce=True):
    ...
    l_ = torch.tensor([random.randrange(0, 32)] * len(batch.C)).to(device) #v2만 사용

    #print('batch C : ', batch.C)
    decoder_outputs_tsf, _ , output_dictionary_tsf = dec(None, hidden_tsf.unsqueeze(0), output_tsf, mask = src_mask, attn_hidden = attn_hidden_tsf, label=l_, gumbel=False,  max_length = max_len) # v2
    #decoder_outputs_tsf, _, output_dictionary_tsf = dec(None, hidden_tsf.unsqueeze(0), output_tsf, mask = src_mask, attn_hidden = attn_hidden_tsf, label=(31-batch.C), gumbel=False,max_length=max_len) #v1
    ...
    loss_r = BCE(logits, l_) #v2
    #loss_r = BCE(logits, 31 - batch.C) #v1
...
def computeOtherLoss2(batch, G, dec, enc_r, attn_r, senti_r, hidden_self, BCE, mse, nll, device, reduce=True, n_c=0):
    max_len = batch.X[1].max().cpu()
    # Style Transfer
    hidden_tsf, output_tsf, attn_hidden_tsf, src_mask, _ = G(batch.X)
    ...
    for i in range(n_c):
        ...
        l_ = torch.tensor([i] * len(batch.C)).to(device) #v3

        decoder_outputs_tsf, _, output_dictionary_tsf = dec(None, hidden_tsf.unsqueeze(0), output_tsf, mask=src_mask, attn_hidden=attn_hidden_tsf, label=l_, gumbel=False, max_length=max_len) #v3
        ...
        loss_r = BCE(logits, l_)

        hidden, output, attn_hidden, src_mask, _ = G(X_style)
        decoder_outputs, decoder_hidden, ret_dict = dec(None, hidden.unsqueeze(0), output, mask=src_mask,
                                                        attn_hidden=attn_hidden, label=batch.C, max_length=max_len)
    ...
    total_loss_r = total_loss_r / n_c
    total_mseLoss = total_mseLoss / n_c
    total_cycle_loss = total_cycle_loss / n_c

    if reduce:
            cycle_loss = total_cycle_loss.sum() / batch.X[1].sum()
            loss_r = total_loss_r.sum() / batch.C.size(0)
            mseLoss = total_mseLoss.sum() / (batch.C.size(0) * mseLoss.size(1))

            return loss_r, mseLoss, cycle_loss


def getLossAndMetrics(iter,G, dec, nll, enc_r, attn_r, senti_r, BCE, mse, config, pad_idx, enc_cls, attn_cls, senti_cls, iter_len, X_VOCAB, C_LABEL, save=False, step=1):
    ...
    for i, batch in enumerate(iter):
        ...
        #loss_r, mseLoss, cycle_loss = computeOtherLoss2(batch, G, dec, enc_r, attn_r, senti_r, hidden_self, BCE, mse, nll, config.device, n_c=32) #v3
        loss_r, mseLoss, cycle_loss = computeOtherLoss(batch, G, dec, enc_r, attn_r, senti_r, hidden_self, BCE, mse,nll, config.device) #v1, #v2
      ...
      #  correct = (preds == (3 - batch.C)).sum().item()

        total += logits.size(0)
        acc += correct
        labels.extend((batch.C).tolist())
     #   labels.extend((3 - batch.C).tolist())
    ...
    if save:
        with open('output/{}/step{}_infer.jsonl'.format(config.name, step), 'w') as fp:
