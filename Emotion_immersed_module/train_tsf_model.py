...
parser.add_argument('--data', type=str, default="Convai2",
                    help='data')
...
G = Generator(enc_cls, attn_cls, enc).to(config.device) #generator에는 encoder layer와 attention만 들어감
...
config.name = name + 'real_v2'

split_tokenizer = lambda x: x.split()
test_ = TabularDataset(path="./data/emotion/test.jsonl",
                  format="json",
                  fields={"X":("X", X_VOCAB),
                          "C": ('C', C_LABEL)})
...
for epoch in tqdm(range(config.max_epoch)):
    for i, batch in enumerate(train_iter):
        ...
        loss_r, mseLoss, cycle_loss = computeOtherLoss(batch, G, dec, enc_r, attn_r, senti_r, hidden_self, BCE, mse, nll, config.device) #v1, v2 사용
        #loss_r, mseLoss, cycle_loss = computeOtherLoss2(batch, G, dec, enc_r, attn_r, senti_r, hidden_self, BCE, mse, nll, config.device, n_c=config.num_class) #v3 사용
        ...
        if global_step%config.eval_iter==0:
            ...
            if loss<minLoss:
                ...
                if not os.path.exists('model/{}'.format(config.name)):
                    os.mkdir('model/{}'.format(config.name))

                if not os.path.exists('output/{}'.format(config.name)):
                    os.mkdir('output/{}'.format(config.name))

                if global_step % 100 == 0 and global_step > 10000: #모델 저장
                    torch.save(G.state_dict(), "model/{}/{}_G.pth".format(config.name, global_step))
                    torch.save(dec.state_dict(), "model/{}/{}_dec.pth".format(config.name, global_step))

                loss, accuracy, bleu_scores= getLossAndMetrics(test_iter,G, dec, nll, enc_r, attn_r, senti_r, BCE, mse, config, pad_idx, enc_cls, attn_cls, senti_cls, len(test), X_VOCAB, C_LABEL, save=True, step=global_step)
                ...
                with open('output/{}/{}.txt'.format(config.name, global_step), 'w') as f:

                    f.write("Total Loss :{}".format(round(loss, 3)))
                    f.write('\n')
                    f.write("self_recon Loss :{}".format(round(self_loss.item(), 3)))
                    f.write('\n')
                    f.write("Cycle Loss :{}".format(round(cycle_loss.item(), 3)))
                    f.write('\n')
                    f.write("Style Loss :{}".format(round(loss_r.item(), 3)))
                    f.write('\n')
                    f.write("content Loss :{}".format(round(mseLoss.item(), 3)))
                    f.write('\n')
                    f.write("Acc :{}".format(round(accuracy*100, 3)))
                    f.write('\n')
                    f.write("BLEU :{}".format(round(bleu_scores, 3)))
