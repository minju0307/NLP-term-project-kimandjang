...
def convert_to_XY(old_data):
    ...
    for i in range(len(old_data)):
        ...
        dialog_hx = list(split_by_index(convo,tokenizer.sep_token_id))

        #if len(dialog_hx) < 30:
        dialog_hx = [line + [tokenizer.sep_token_id] for line in dialog_hx[:20]] # limit by max len of convo
        p1_ctx = tokenizer.encode(''.join(['<|p1|>'] + p1 + ['<|sep|>'] + ['<|start|>']))
        p2_ctx = tokenizer.encode(''.join(['<|p2|>'] + p2 + ['<|sep|>'] + ['<|start|>']))
        for t in range(len(dialog_hx)):
            x = dialog_hx[:t] ## 여기서 배치처리하는 거 처리하면 될 듯.
            y = dialog_hx[t]
            if t == 0:
                x = p1_ctx[:-1]
                y = [p1_ctx[-1]] + y
            elif t %2 ==0:
                x = p1_ctx + flatten(x) ## p1의 대화
            else:
                x = p2_ctx + flatten(x) ## p2의 대화
            ...
            x_pad_tok_list=[pad_tok_id for _ in range(x_pad_len)]
            y_pad_tok_list = [pad_tok_id for _ in range(y_pad_len)]
            x=x+x_pad_tok_list
            y=y+y_pad_tok_list
            print(f'x: {x}')
            print(f'y_final: {y}')
            data.append((x,y)) # y를 길이에 맞춰서 넣어줘야 할듯
            print(f'x len: {len(x)}')
            print(f'y len: {len(y)}')
            assert len(x)==455
            assert len(y)==67
