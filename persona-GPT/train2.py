...
from load_configs import model, tokenizer, pretrain_stats, train_stats, opts, create_dir, p1_tok, p2_tok, start_tok, act_tok
from utils import *
...
def checkpoint(model, tokenizer, optimizer, scheduler, stats, title="", cp=""):
    output_dir='/home/isds1/persona-persona/personaGPT-/ckpt/checkpoint'
    output_dir=os.path.join(output_dir, cp)
...
def fit_on_batch(batch):
    ...
    label=[[y if y !=50258 else -100 for y in _yy] for _yy in yy]
    # print(f'yy: {yy}')
    # print()
    # print()
...
def pretrain(data, stats=None):
    ...
    model, optimizer, dataloader, schedulear = accelerator.prepare(model, optimizer, dataloader, scheduler)
    ...
    for epoch in range(epochs_trained, opts.num_train_epochs):
        ...
        if global_step % 5000 == 0:
            print("Saving stuff ... ")
            checkpoint(model, tokenizer, optimizer, scheduler, stats, title="pretrain_", cp=str(epoch))
